class Solution {
public:
    void buildSegmentTree(int i, int l, int r, int segmentTree[], const int arr[]) {
        if (l == r) {
            segmentTree[i] = arr[l];
            return;
        }
    ...
    int* constructST(const int arr[], int n) {
        ...
        for (auto& q : queries) {
            ...
            if (low < high) {
                ...
                if (high - low == 1) {
                    ...
                    int p1 = firstLen + blockSize[low + 1];
                    int p2 = lastLen + blockSize[high - 1];

                    int midMax = INT_MIN;

                    if (low + 1 <= high - 2)
                        midMax = RMQ(st, N, low + 1, high - 2);

                    maxPairSum = max({p1, p2, midMax});
                }
            }
